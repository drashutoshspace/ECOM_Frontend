...
import { updateReview, deleteReview, reviewratingForm } from "../../helpers/others/reviewratingForm";
...
const ProductSingle = (props) => {
	const { findProduct, product } = useContext(ProductsContext);
	...
	var stars = [];
	var showStars1 = (number) => {
		for (var i = 0; i < number; i++) {
			stars.push(
				<li key={i} className="list-inline-item fontsize18 hvr-icon-grow me-1">
					<i className="fas fa-star hvr-icon" />
				</li>
			);
		}
		return <ul className="list-unstyled text-center text-lg-start coloryellow mb-3">{stars}</ul>;
	};
	/* -------------------------------------------------------------------------- */
	...
	var showStarsForRating = (number) => {
		var starsForRating = [];
		for (var i = 0; i < number; i++) {
			starsForRating.push(<i key={i} className="fas fontsize18 fa-star coloryellow" />);
		}
	...
	var showStarsForRatingForm = (number, indexValue) => {
		...
		for (var i = 0; i < number; i++) {
			starsForRatingForm.push(<i className={`${indexValue === target ? "fas fa-star coloryellow hvr-icon" : "far fa-star"}`} />);
		}
	...
	const reviewrating = async (e) => {
		...
		if (localStorage.getItem("token")) {
			...
			await reviewratingForm({ product_id: product.guid, review, rating }).then((data) => {
				if (data.user) {
	...
	useEffect(() => {
		singleProduct(props.match.params.id, (data) => {
			findProduct(data);
			revs(data);
			setLoading(false);
		});
	...
	return (
		<>
			<Helmet title={`Kirana For Home | ${product?.Product_Name}`}></Helmet>
			<Base>
				{loading ? (
					<DataLoader />
				) : (
						...
						<div className="container">
							<div className="row">
								<div className="col-lg-6">
									<div className="mb-3 card border-0 shadow">
										...
										<span className="d-flex justify-content-center align-items-center" id="mydiscountpersin">
											{Math.abs(parseInt(product?.Product_Discount) - parseFloat(product?.Product_Discount)) > 0.5
										...
										<button className="d-flex hvr-icon-pulse border-0 justify-content-center align-items-center mywishlistsin heartredhover">
											<div className={`${wishlistToggle ? "fas fa-2x fa-heart heartred hvr-icon" : "far fa-2x fa-heart"}`} onClick={changeWishlist} />
										</button>
									</div>
									<div className="row">
										{product.Product_Images?.slice(0, 3).map((product, index) => {
											return (
												<>
													{product === product.Product_Images?.slice(-1)[0] ? (
														<div key={index} className="col-3">
															<img
																className="border5px h-100 w-100 shadow cursorpointer"
																onClick={() => setProductImage1(index)}
																src={`${product?.dbImage || tempImg}`}
																alt=""
															/>
														</div>
													) : (
								...
								<div className="col-lg-6 ps-lg-4">
									<h1 className="colorblue mb-2 mt-3 mt-lg-0 text-center fw-bold text-lg-start">{product?.Product_Name}</h1>
									{showStars1(
										Math.abs(parseInt(product?.Product_Rating) - parseFloat(product?.Product_Rating)) > 0.5
											? parseInt(product?.Product_Rating) + 1
											: parseInt(product?.Product_Rating)
									)}
									<p className="mb-3 text-center text-lg-start">
										<span className="colorblue fw-bold" style={{ fontSize: 28 }}>
											₹ {product?.Product_SellingPrice}
										</span>
										&nbsp;&nbsp;
										<span className="fontsize20 notaccepted text-decoration-line-through">₹ {product?.Product_MRP}</span>
									</p>
									<div className="row">
										<div className="col d-flex justify-content-center align-items-center">
											<button className="h-100 w-75 colorblue fontsize20 border-0 border5px bgyellow bglightblue" onClick={handleMinus}>
											...
											/>
											<button className="h-100 w-75 colorblue fontsize20 border-0 border5px bgyellow bglightblue" onClick={handlePlus}>
									...
									<ul className="fontsize18 mb-2 mt-4 list-unstyled text-start">
										<li className="my-1" style={{ borderBottom: "2.5px dotted #ebebeb" }}>
											<span className="fw-bold colorblue">Brand :</span>
											<span className="colorlightblue">&nbsp;&nbsp;&nbsp;{product?.Product_Brand}</span>
										</li>
										<li className="my-1" style={{ borderBottom: "2.5px dotted #ebebeb" }}>
											<span className="fw-bold colorblue">Specifications :</span>
											<span className="colorlightblue">&nbsp;&nbsp;&nbsp;{product?.Product_Specs}</span>
										</li>
										<li className="my-1" style={{ borderBottom: "2.5px dotted #ebebeb" }}>
											<span className="fw-bold colorblue">Category :</span>
											<span className="colorlightblue">&nbsp;&nbsp;&nbsp;Electronic Components, Small Components</span>
										</li>
										<li className="my-1" style={{ borderBottom: "2.5px dotted #ebebeb" }}>
											<span className="fw-bold colorblue">SKU :</span>
											<span className="colorlightblue">&nbsp;&nbsp;&nbsp;SHC0449-1</span>
										</li>
										<li className="my-1" style={{ borderBottom: "2.5px dotted #ebebeb" }}>
											<span className="fw-bold colorblue">Contents :</span>
											<span className="colorlightblue">&nbsp;&nbsp;&nbsp;hjdhe, uidhnw, dwjjd</span>
										</li>
										<li className="my-1" style={{ borderBottom: "2.5px dotted #ebebeb" }}>
											<span className="fw-bold colorblue">Quantity :</span>
											<span className="colorlightblue">&nbsp;&nbsp;&nbsp;100</span>
										</li>
										<li className="mt-3 mt-lg-2 text-center">
											<WhatsappShareButton className="me-3 hvr-icon-float" url={`https://www.kiranaforhome.com${location.pathname}`}>
												<img className="hvr-icon" height="25px" alt="WhatsApp" src="images/Logo/WA_Logo.svg" />
											</WhatsappShareButton>
											<FacebookShareButton className="me-3 hvr-icon-float" url={`https://www.kiranaforhome.com${location.pathname}`}>
												<img className="hvr-icon" height="25px" alt="Facebook" src="images/Logo/FB_Logo.svg" />
											</FacebookShareButton>
											<TelegramShareButton className="me-3 hvr-icon-float" url={`https://www.kiranaforhome.com${location.pathname}`}>
												<img className="hvr-icon" height="25px" alt="Telegram" src="images/Logo/Telegram.svg" />
											</TelegramShareButton>
											<TwitterShareButton className="me-3 hvr-icon-float" url={`https://www.kiranaforhome.com${location.pathname}`}>
												<img className="hvr-icon" height="25px" alt="Twitter" src="images/Logo/Twitter.svg" />
											</TwitterShareButton>
											<LinkedinShareButton className="hvr-icon-float" url={`https://www.kiranaforhome.com${location.pathname}`}>
												<img className="hvr-icon" height="25px" alt="LinkedIn" src="images/Logo/LinkedIn.svg" />
											</LinkedinShareButton>
										</li>
									</ul>
								</div>
							...
							<div className="row">
								<div className="col-lg-12 mt-3">
									<ul className="nav d-flex justify-content-center nav-pills mb-3" id="pills-tab" style={{ borderBottom: "2.5px dotted #ebebeb" }} role="tablist">
										<li className="nav-item" role="presentation">
											<button
												...
												aria-selected="true"
												style={{ border: "2.5px", borderRadius: "0px", borderRight: "2.5px dotted #ebebeb" }}
											>
												Description
											</button>
										</li>
									...
									<div className="tab-content" id="pills-tabContent">
										<div className="tab-pane fade show mypara text-center active" id="pills-description" role="tabpanel" aria-labelledby="pills-description-tab">
											{product?.Product_Description?.length > 0 ? (
												product?.Product_Description?.length
											) : (
												<h2 className="colorblue pt-3 text-center">No description added by the administrator!</h2>
											)}
										</div>
										<div
										...
										>
											{allReviews.filter((rev) => rev.product_id === product?.guid).length > 0 ? (
												allReviews
													.sort((a, b) => {
														return a.rating - b.rating;
													})
													.reverse()
													.filter((rev) => rev.product_id === product?.guid)
													.map((review, index) => {
														return (
															<div key={index} className="row my-4">
																<div className="col-12">
																	<div className="teacher d-flex justify-content-end align-items-center">
																		<img src={review?.user?.image || tempImg1} className="avatar-md-lg rounded-circle shadow" alt="" />
																		<div className="ms-3 flex-grow-1">
																			<h4 className="mb-1">
																				<p className="mb-0 colorblue">
																					{review?.user.first_name !== "" ? review?.user.first_name : review?.user.username}
																					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
																					{showStarsForRating(review?.rating)}
																				</p>
																			</h4>
																			<p className="colorblue mb-0 mypara">{review?.review}</p>
																		</div>
																		{review?.user?.id === cookies?.user?.[0]?.id && (
																			<button
																				onClick={(e) => {
																					reviewDelete(e, review.id);
																				}}
												...
												<div className="row mt-5">
													<div className="col-lg-12 text-center">
														<img width="250px" src={changeImage1 ? "images/No_Reviews_Yellow.svg" : "images/No_Reviews_LightBlue.svg"} className="loginsvg" alt="" />
														<h3 className="mt-4 pt-3 text-center colorblue letterspacing1">There are no reviews to show as of now!</h3>
													</div>
												</div>
											)}
										</div>
